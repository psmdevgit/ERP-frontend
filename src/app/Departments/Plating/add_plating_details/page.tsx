    ...
    const initializePlating = async () => {
      ...
      try {
        ...
        console.log('[Add Plating] Fetching pouches from:', {
          url: `${API_BASE_URL}/api/dull/${prefix}/${date}/${month}/${year}/${number}/${subnumber}/pouches`
        });

        const pouchResponse = await fetch(
          `${API_BASE_URL}/api/dull/${prefix}/${date}/${month}/${year}/${number}/${subnumber}/pouches`
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[Add Plating] Submitting to API:', {
        url: `${API_BASE_URL}/api/plating/create`,
        data: JSON.stringify(platingData, null, 2)
      });

      const response = await fetch(`${API_BASE_URL}/api/plating/create`, {
        method: 'POST',
        headers: { 
